...
function buildSVG({ name, text, tag, avatar, handle, isUnknown }) {
  ...
  // 프로필 영역 — 이미지 or 이니셜 원
  const profileBlock = isUnknown
    ? `<!-- 이니셜 원 (미등록 캐릭터) -->
  <circle cx="44" cy="52" r="24" fill="#1e2028"/>
  <circle cx="44" cy="52" r="24" fill="none" stroke="#2a2d35" stroke-width="1.2"/>
  <text x="44" y="57"
        font-family="'Apple SD Gothic Neo','Noto Sans KR',sans-serif"
        font-size="18" font-weight="700" fill="#536471"
        text-anchor="middle">${esc(getInitial(name))}</text>`
    : `<!-- 프로필 이미지 -->
  <image href="${esc(avatar)}"
         x="20" y="28" width="48" height="48"
...
export default {
  async fetch(request) {
    ...
    const char   = CHARACTERS[name]
    const avatar = char
      ? `${GITHUB_IMG_BASE}/${char.id}.png`
      : `${GITHUB_IMG_BASE}/default.png`
    const handle = char?.handle ?? "@unknown"

    const svg = buildSVG({ name, text, tag, avatar, handle })
